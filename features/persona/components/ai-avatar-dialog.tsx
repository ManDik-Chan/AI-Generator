...
export function AiAvatarDialog({ open, onOpenChange, onApplied, personaId, personaName, currentAvatarUrl, initialPrompt = "", configured }: Props) {
  ...
  if (!open) return null;
  return <div aria-labelledby="ai-avatar-title" aria-modal="true" className="mobile-modal-overlay" onClick={() => { if (!applying) void close(); }} role="dialog">
    <div className="mobile-modal-panel premium-panel-strong premium-scrollbar max-w-2xl overflow-y-auto p-4 sm:p-6" onClick={(event) => event.stopPropagation()}>
      <div className="sticky top-0 z-10 -mx-1 flex items-start justify-between gap-3 border-b border-border/10 bg-surface-raised/95 px-1 pb-4"><div><p className="premium-kicker">AI AVATAR STUDIO</p><h2 className="mt-1 text-xl font-semibold tracking-[-.025em]" id="ai-avatar-title">AI 生成头像</h2><p className="mt-1 text-sm text-muted-foreground">仅在点击生成后调用一次；应用前不会改变当前头像。</p></div><Button aria-label="关闭" disabled={applying} onClick={() => void close()} size="icon" type="button" variant="ghost"><X className="size-4" /></Button></div>
