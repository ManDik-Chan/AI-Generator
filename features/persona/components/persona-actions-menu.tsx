...
export function PersonaActionsMenu({ personaId, personaName, archived }: { personaId: string; personaName: string; archived: boolean }) {
  ...
  return <div className="relative shrink-0">
    ...
    {menuOpen && <div className="absolute right-0 top-12 z-20 w-48 rounded-xl border bg-card p-1.5 shadow-xl" role="menu"><Button asChild className="w-full justify-start" size="sm" variant="ghost"><Link href={`/personas/${personaId}/edit`}><Pencil className="size-4" />编辑人格</Link></Button>{archived ? <Button className="w-full justify-start" disabled={pending} onClick={restore} size="sm" type="button" variant="ghost"><RotateCcw className="size-4" />恢复人格</Button> : <Button className="w-full justify-start text-red-600 hover:text-red-700" onClick={() => { setMenuOpen(false); setDeleteOpen(true); }} size="sm" type="button" variant="ghost"><Trash2 className="size-4" />删除人格</Button>}{error && <p className="px-2 py-1 text-xs text-red-600">{error}</p>}</div>}
