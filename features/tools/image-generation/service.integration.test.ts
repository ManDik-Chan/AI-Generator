...
describe("tool image generation orchestration", () => {
  beforeEach(() => {
    ...
    mocks.upload.mockImplementation(async (userId: string, path: string) => ({ bucket: "generated-images", path, kind: "TOOL_GENERATION", userId }));
  ...
  it("deletes only an owned tool generation and its ToolRun", async () => {
    ...
    expect(mocks.remove).toHaveBeenCalledWith(expect.objectContaining({ bucket: "generated-images", path: "owner/image.png", kind: "TOOL_GENERATION" }));
  ...
  it("does not delete from a database-forged bucket", async () => {
    mocks.imageFind.mockResolvedValue({ id: "image", storageBucket: "other-private-bucket", storagePath: "owner/image.png", toolRunId: "run" });
    await expect(deleteToolGeneratedImage("owner", "image")).resolves.toBe("not-found");
    expect(mocks.remove).not.toHaveBeenCalled();
    expect(mocks.runDelete).not.toHaveBeenCalled();
  });
});
