...
export function BrainstormWorkspace({ configured, initialUsage }: { configured: boolean; initialUsage: BrainstormUsageDto }) {
  ...
  return <div className="min-w-0 space-y-8">
    <section className="premium-panel min-w-0 p-4 sm:p-6">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-border/10 pt-4"><div className="flex flex-wrap gap-2">{active ? <Button disabled={cancelling} onClick={() => void stop()} variant="outline"><Square className="size-4 fill-current" />{cancelling ? "正在请求停止" : "停止头脑风暴"}</Button> : <Button disabled={!configured || !prompt.trim()} onClick={() => void start()}><BrainCircuit className="size-4" />开始头脑风暴</Button>}{state !== "idle" && !active ? <Button onClick={reuse} variant="outline"><RotateCcw className="size-4" />再次头脑风暴</Button> : null}</div><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">{active && <span>{formatElapsedTime(elapsed)}</span>}{recoveryMessage && <span className="premium-chip"><LoaderCircle className="size-3 animate-spin motion-reduce:animate-none" />{recoveryMessage}</span>}</div></div>
    </section>

    <section><div><p className="premium-kicker">INDEPENDENT WORKERS</p><h2 className="mt-1 text-section-title">四个 Worker</h2><p className="mt-1 text-supporting">四个角色独立调用，单个失败不会阻止其他角色继续。</p></div><div className="mt-4 grid min-w-0 gap-4 md:grid-cols-2">{workers.map((worker) => { const Icon = icons[worker.role]; const running = active && worker.status === "PENDING" && Boolean(worker.startedAt); return <article className="premium-panel flex min-h-64 min-w-0 flex-col p-4 sm:p-5" key={worker.role}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="premium-icon-tile size-11 shrink-0"><Icon className="size-5" /></span><div className="min-w-0"><p className="font-semibold">{worker.label}</p><p className="mt-1 text-xs text-muted-foreground">{BRAINSTORM_WORKERS.find((item) => item.role === worker.role)?.purpose}</p></div></div><span className={`premium-chip shrink-0 ${statusClass(worker.status, running)}`}>{workerStatus(worker.status, running)}</span></div><div className="premium-result premium-scrollbar mt-4 max-h-[28rem] min-h-36 flex-1 overflow-y-auto p-4">{worker.output ? <MarkdownMessage content={worker.output} /> : worker.status === "ERROR" ? <p className="text-sm text-destructive-foreground">该 Worker 运行失败，没有可展示的输出。</p> : <p className="text-sm text-muted-foreground">{running ? "正在独立分析…" : active ? "等待可用并发位置…" : "运行后将在这里显示真实输出。"}</p>}</div>{worker.output && <Button className="mt-3 self-start" onClick={() => void navigator.clipboard.writeText(worker.output)} size="sm" variant="ghost"><Copy className="size-3.5" />复制</Button>}</article>; })}</div></section>
