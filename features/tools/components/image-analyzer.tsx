...
export function ImageAnalyzer({ configured, initialRemaining, initialLimit, initialUsed, initialUnlimited }: Props) {
  ...
  return <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)]">
    <section className="premium-panel min-w-0 p-4 sm:p-6">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-border/10 pt-4 max-[359px]:[&>button]:w-full">{active ? <Button disabled={cancelling} onClick={() => void stop()} variant="outline"><Square className="size-4" />{cancelling ? "正在请求停止" : "停止分析"}</Button> : <Button disabled={!preview || (options.mode === "question" && !question.trim())} onClick={() => void run()}><WandSparkles className="size-4" />开始分析</Button>}<span className="premium-chip max-w-full overflow-wrap-anywhere">{statusLabel}</span></div>
    </section>
