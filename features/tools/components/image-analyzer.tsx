...
export function ImageAnalyzer({ configured, initialRemaining, initialLimit, initialUsed, initialUnlimited }: Props) {
  ...
  const [runId, setRunId] = useState<string>(); const [usage, setUsage] = useState({ limit: initialLimit, used: initialUsed, remaining: initialRemaining, unlimited: initialUnlimited }); const [copied, setCopied] = useState(false);
  ...
  async function run() {
    ...
    try {
      ...
      await readSseEvents(response, (event, data) => { const payload = data as Record<string, unknown>; if (event === "start") { setRunId(String(payload.runId)); setUsage({ limit: Number(payload.limit), used: Number(payload.used), remaining: Number(payload.remaining), unlimited: payload.unlimited === true }); setState("streaming"); } if (event === "delta") { setState("streaming"); setOutput((current) => current + String(payload.text || "")); } if (event === "done") { terminal = true; setState("complete"); } if (event === "error") { terminal = true; if (payload.code === "CANCELLED") setState("stopped"); else { setState("error"); setError(String(payload.message || "图片分析失败，请重试。")); } } });
  ...
  return <div className="grid min-w-0 gap-5 lg:grid-cols-2">
    <section className="min-w-0 rounded-2xl border bg-card p-4 shadow-soft sm:p-6">
      <div className="flex items-center justify-between gap-3"><p className="font-medium">上传图片</p><p className="text-right text-xs text-muted-foreground">{formatVisionUsage(usage)}</p></div>
