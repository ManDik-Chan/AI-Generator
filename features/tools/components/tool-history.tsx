...
export function ToolHistory({ items, page, pages, filter }: { items: ToolRunListItem[]; page: number; pages: number; filter: "ALL" | ToolTypeValue }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2">{(["ALL", "SUMMARIZE", "REWRITE", "TRANSLATE", "IMAGE_ANALYZE"] as const).map((type) => <Button asChild key={type} size="sm" variant={filter === type ? "default" : "outline"}><Link href={type === "ALL" ? "/tools/history" : `/tools/history?type=${type}`}>{type === "ALL" ? "全部" : TOOL_LABELS[type]}</Link></Button>)}</div>
    {visible.length ? <div className="grid gap-4">{visible.map((item) => <article className="min-w-0 rounded-2xl border bg-card p-4" key={item.id}>
      <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium text-primary">{TOOL_LABELS[item.type]} · {item.status === "COMPLETE" ? "已完成" : item.status === "CANCELLED" ? "已停止" : item.status === "ERROR" ? "失败" : "处理中"}</p><h2 className="mt-1 break-words font-semibold">{item.title || TOOL_LABELS[item.type]}</h2><p className="mt-1 text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString("zh-CN")}</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => void open(item.id)} size="sm" variant="outline"><ExternalLink className="size-3.5" />{details[item.id] ? "收起" : "打开"}</Button><Button onClick={() => void continueEditing(item.id)} size="sm" variant="ghost"><Pencil className="size-3.5" />{item.type === "IMAGE_ANALYZE" ? "再次分析" : "继续编辑"}</Button><Button className="text-red-600" onClick={() => void remove(item.id)} size="sm" variant="ghost"><Trash2 className="size-3.5" />删除</Button></div></div>
      <div className="mt-3 grid min-w-0 gap-3 text-sm sm:grid-cols-2"><div className="min-w-0 break-words rounded-xl bg-muted/60 p-3"><span className="mb-1 block text-xs text-muted-foreground">输入</span>{item.type === "IMAGE_ANALYZE" && item.asset ? item.asset.expired ? <span>原图片已到期清理。</span> : <span className="relative mt-2 block aspect-video overflow-hidden rounded-lg"><span className="absolute inset-0 grid place-items-center text-xs text-muted-foreground">图片暂时不可用。</span><Image alt="历史图片缩略图" className="z-10 bg-muted object-contain" fill onError={(event) => { event.currentTarget.style.visibility = "hidden"; }} src={`/api/tools/assets/${item.asset.id}`} unoptimized /></span> : item.inputPreview || "无可恢复输入"}</div><p className="min-w-0 break-words rounded-xl bg-muted/60 p-3"><span className="mb-1 block text-xs text-muted-foreground">输出</span>{item.outputPreview || (item.status === "ERROR" ? "运行失败，没有完整输出。" : item.status === "CANCELLED" ? "运行已停止，没有保存完整输出。" : "暂无输出")}</p></div>
