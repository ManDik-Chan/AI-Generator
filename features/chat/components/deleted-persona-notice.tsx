...
export function DeletedPersonaNotice({ personaId }: { personaId: string }) {
  ...
  return <div className="border-b border-amber-500/25 bg-amber-500/10 px-4 py-2 text-sm text-amber-900 dark:text-amber-100"><div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-2"><span>该人格已在回收站，恢复后可以继续对话。</span><Button disabled={pending} onClick={() => startTransition(async () => { setError(undefined); const result = await restorePersonaFromTrash(personaId); if (!result.success) setError(result.message); else router.refresh(); })} size="sm" type="button" variant="outline"><RotateCcw className="size-3.5" />{pending ? "正在恢复…" : "恢复人格"}</Button>{error && <span className="text-red-700 dark:text-red-300">{error}</span>}</div></div>;
}
