...
export function AssistantSelectorPanel({ personas, selectedId, mobile = false, onClose, onSelect }: AssistantSelectorPanelProps) {
  ...
  if (mobile) return <div aria-labelledby="mobile-assistant-selector-title" aria-modal="true" className="fixed inset-0 z-50 overflow-hidden bg-overlay/55 backdrop-blur-sm xl:hidden" onClick={onClose} role="dialog"><aside className="ml-auto flex h-[var(--visual-viewport-height)] w-[min(90vw,21rem)] max-w-[calc(100vw-var(--safe-area-left)-.5rem)] flex-col border-l border-border/10 bg-background-subtle pb-[var(--safe-area-bottom)] shadow-2xl" onClick={(event) => event.stopPropagation()}><span className="sr-only" id="mobile-assistant-selector-title">选择助手</span>{content}</aside></div>;
