...
export function AssistantSelectorPanel({ personas, selectedId, mobile = false, onClose, onSelect }: AssistantSelectorPanelProps) {
  ...
  if (mobile) return <div aria-labelledby="mobile-assistant-selector-title" aria-modal="true" className="fixed inset-0 z-50 bg-overlay/55 backdrop-blur-sm xl:hidden" onClick={onClose} role="dialog"><aside className="ml-auto flex h-full w-[min(90vw,21rem)] flex-col border-l border-border/10 bg-background-subtle shadow-2xl" onClick={(event) => event.stopPropagation()}><span className="sr-only" id="mobile-assistant-selector-title">选择助手</span>{content}</aside></div>;
