...
const mocks = vi.hoisted(() => ({ getUser: vi.fn(), configured: vi.fn(), getProvider: vi.fn(), generate: vi.fn(), createRun: vi.fn(), pending: vi.fn(), finish: vi.fn() }));
...
describe("persona generation API", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.createRun.mockResolvedValue({ id: "run" }); mocks.pending.mockResolvedValue(true); mocks.finish.mockResolvedValue({ count: 1 }); mocks.configured.mockReturnValue({ configured: true }); mocks.getProvider.mockReturnValue({ config: { model: "m", temperature: 0.8, maxOutputTokens: 1800 }, provider: {} }); });
