...
本说明适用于 migration `20260722120000_security_hardening_rls_usage`。本开发任务没有连接或修改 Production，也没有执行任何 Production migration。
...
4. 暂停 Preview 写流量，避免账本回填与旧应用同时创建未入账的新记录。
...
`pnpm db:deploy` 会在同一个版本化 migration 中创建并回填 `usage_ledger`，部署 Profile 列级权限、运行状态表只读 grants、RLS policies 和触发器。正常发布不得再人工粘贴 `prisma/rls.sql`；它只用于数据库恢复后的完整基准重建。
...
- migration 使用显式事务；任何 SQL 失败应整体回滚。
- 回填按既有 Message、ToolRun、AgentRun 创建不可由浏览器修改的账本行，`ON CONFLICT DO NOTHING` 保证重复恢复操作不重复记账。
- 新应用必须在 migration 成功后发布，否则 Prisma Client 查询不存在的 `usage_ledger` 会失败。
