...
- [x] Phase 5A1：长期记忆基础、手动管理、安全召回与聊天注入
- [ ] Phase 5A2 / 5A3：尚未开始
...
Phase 5A1 已完成代码实现与自动化验证：增加 Memory 作用域、来源、最近使用时间和用户总开关；实现 `/memories` CRUD、聊天消息手动保存、GLOBAL/PERSONA 隔离、凭据拦截、确定性预算召回、安全 Prompt 注入、memory SSE 数量与 COMPLETE 后 `lastUsedAt` 更新。没有实现自动提取、Embedding、向量数据库或 RAG。

真实 Supabase migration 与 RLS 多用户验收尚待项目所有者执行，因此本记录不声明真实数据库联调已通过。
...
- [ ] Phase 5：记忆系统（5A1 代码实现完成，真实环境待验收）
- [ ] Phase 6：工具箱
- [ ] Phase 7：优化与部署
...
- 扩展现有 Memory 数据模型：GLOBAL/PERSONA、MANUAL/CHAT_MESSAGE 来源、Persona、来源消息、`lastUsedAt` 与 Profile 总开关。
- migration 对旧数据回填 GLOBAL/MANUAL，并增加关系约束、检查约束和查询索引；RLS 同时校验 Memory 所有者与关联 Persona/Conversation/Message 所有者。
- `/memories` 支持创建、编辑、启停、删除、筛选和总开关；删除记忆不会删除来源聊天或 Persona。
- 仅已持久化的 COMPLETE 用户消息提供“保存为记忆”，服务端再次验证消息角色、状态、未被 supersede 和对话所有权。
