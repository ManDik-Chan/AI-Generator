...
Phase 6A1 提供 `/tools`、`/tools/summarize`、`/tools/rewrite`、`/tools/translate` 与 `/tools/history`。本节只描述文本工具，不上传或解析 PDF/DOCX/PPTX/图片，不做 OCR、网页抓取、搜索、文件/网页 RAG、医疗诊断、法律咨询、旅行规划或图片生成。后续 Phase 6A2 图片工具已完成真实验收。
...
Phase 6A1 已完成真实验收。文本工具仍不创建 Conversation/Message、不读取或写入长期记忆、不绑定 Persona；不包含文件上传、OCR、文件解析或 RAG。

## Phase 6A2 扩展

图片分析复用 ToolRun 的 SSE/取消/终态保护，但文件先经服务端净化，再创建计费运行。资源使用独立 ToolAsset 与 private Storage，不进入 JSON 历史。视觉次数独立于文本工具与聊天次数。真实 migration/RLS/private Storage、视觉分析、停止、历史、清理、隔离、Prompt 注入、管理员额度显示与响应式验收已全部通过。详细安全、部署和验收见 `image-understanding-tool.md`。

Phase 6A2 不包含 PDF/DOCX、专业 OCR、RAG、多图、视频、图片生成或编辑。Phase 6A3 与 Phase 7 尚未开始。

## Phase 6A3 扩展

`IMAGE_GENERATE` 使用同一 ToolRun 状态机与应用 SSE，但拥有独立每日限额、独立 private bucket 和一对一 `GeneratedImage` 结果。合法请求在创建 PENDING 运行时计数；停止与错误同样计数，非法参数和缺少配置不会创建运行。工具图片不会进入聊天、Persona 或长期记忆，Persona 头像也不会出现在工具画廊。当前为 Draft 真实验收状态，详细安全边界见 `image-generation-tool.md`。
