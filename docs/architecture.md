...
## Phase 6B3 Lumen 应用壳与 Chat 视口

Lumen Token、排版、圆角、阴影和轻量 CSS 动效集中在 `app/globals.css` 与 Tailwind 扩展。`AppShell` 组合 252 px Desktop Sidebar、Workspace Topbar、Mobile Header、完整移动菜单和五项移动主导航；固定路由进入统一命令目录，ADMIN 入口由真实 Profile role 过滤。Home 主体继续是 Server Component，个性化区在 Suspense 中读取，认证和 Shell viewer 使用 React `cache()` 做单请求去重，增长型动态详情关闭自动预取。

普通 Home、Auth、Persona、Memory、Tools、Account 与 Admin 在所有宽度都使用 document/body 自然滚动；Sidebar 与移动导航可以固定，但页面根不消费 VisualViewport。`scrollMode="viewport"` 只为有明确理由的受限工作区保留。

Chat 不使用普通 AppShell。`ChatLayout` 挂载唯一的 `useChatVisualViewport` client controller；它只在 820 px 以下读取 VisualViewport，并把 `--chat-viewport-top`、`--chat-viewport-height`、`--keyboard-inset` 写到 Chat shell。手机 shell fixed 到当前可视区域，body 不滚动，消息列表是唯一主滚动区，Composer 在 flex 流末端。resize/scroll 共用单个 rAF，小于 1 px 的变化不写，hidden 时暂停，visible/pageshow 校准，卸载清理变量和监听。Composer 与 MessageList ResizeObserver 也只存在于 Chat feature。

浮层属于共享 UI 层。Dialog 桌面居中、手机贴底，背景锁计数与恢复不调用 `window.scrollTo`；Dropdown Portal 到 `document.body`，以 VisualViewport 边界执行 12 px collision padding、上下翻转与左右夹紧。以上显示层变化不接触 ToolRun、Message、GenerationRun、Provider、数据库或鉴权边界。
