...
根布局只挂载一个无视觉输出的 `MobileViewportSync`。它仅在 820px 以下注册 VisualViewport、resize 和 focus 监听；resize 可同步高度，VisualViewport scroll 只同步 offset，单个 `requestAnimationFrame` 合并写入且小于 1px 的变化被忽略，不触发 React 页面树重渲染。页面隐藏时停止同步，visible/pageshow 时校准一次。没有 VisualViewport 的浏览器回退到 `innerHeight` 与 CSS `svh` / `dvh`。

AppShell 显式区分 document 与 viewport：手机普通页使用固定 app viewport、独立 Header、唯一可滚动 Main 与浮动导航；桌面普通页使用 document/body 自然滚动和固定 Sidebar。Chat 保持独立固定 viewport，以消息列表作为唯一滚动区、Composer 作为底部固定层；输入聚焦不主动移动文档，路由卸载和页面隐藏不改变 durable generation 状态。
...
Phase 6B2.1 将首页主体迁移为 Server Component，个性化数据在 Suspense 中流式渲染；认证和 Shell viewer 通过 React `cache()` 做请求内去重，Viewer 与最近对话并行读取。首页移除 Framer Motion，增长型动态详情 Link 关闭自动预取，主要路由用稳定 loading 边界和全局导航反馈提示切换。这些优化不跨用户缓存数据，也不改变认证或业务查询语义。
