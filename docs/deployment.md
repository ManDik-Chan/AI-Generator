...
2. 重新执行最新版 `prisma/rls.sql`，确认工具 GeneratedImage 只能绑定当前用户自己的 ToolRun。
...
5. 真实验收预览、下载、删除、停止补偿、跨用户隔离、额度、Prompt 注入和 390/430/768/1440px。日志不得包含 Prompt、远程临时 URL、Storage path、signed URL、密钥或图片字节。
