...
## Phase 7A1.1 Agent Mode 部署补充

Phase 7A1.1 新增 migration `20260718160000_add_agent_workers`。本地开发任务没有连接或修改生产数据库；以下步骤必须先在隔离的 Preview 数据库执行并由项目所有者验收：

1. 备份 Preview 数据库，部署目标分支代码，执行 `pnpm exec prisma generate`。
2. 使用 Preview 的迁移连接执行 `pnpm db:deploy`，再执行 `pnpm exec prisma migrate status`，确认历史 migration 与 `20260718160000_add_agent_workers` 均已应用且没有 drift。
3. 在 Supabase SQL Editor 完整重复执行最新版 `prisma/rls.sql`；确认 `agent_runs`、`agent_workers`、`agent_worker_events` 只有 select-own 客户端策略，INSERT/UPDATE/DELETE 继续由 trusted server 负责。
4. 仅在 Vercel 服务端环境配置 `.env.example` 中的 `AI_AGENT_*`、`AI_DAILY_AGENT_CREDITS`、Base URL 与 API Key。任何 Key、数据库 URL、Prompt 或 Worker 输出都不得使用 `NEXT_PUBLIC_*` 或进入构建日志。
5. 重新部署 Preview，验证 Standard/Deep、fallback、部分失败、依赖等待、单 Worker/全局取消、断线恢复、deadline、跨用户 404、credits、普通追问和 `/agents` 历史。
6. 只有在 Preview、真实服务端模型调用和项目所有者验收通过后才合并；本分支不执行 Production migration、RLS、域名或部署操作。正式域名保持 `https://www.ai-mdc.com`。

回滚应用时先禁止新的 Agent 创建并等待或显式取消 PENDING 运行，再回滚代码。数据库结构默认保留以便前滚修复；若最终必须移除，只能新增经过审查的补偿 migration，先确认无 AgentRun/Message 关联数据，禁止编辑或删除已经部署的 migration。Classic Brainstorm 不依赖本 migration，回滚 Agent Mode 不应影响 `/tools/brainstorm`。
