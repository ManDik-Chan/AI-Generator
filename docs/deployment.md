...
Phase 7A1 需要部署独立 migration `20260717180000_add_brainstorm_workers`，随后在 Supabase SQL Editor 重复执行最新版 `prisma/rls.sql`。配置仅服务端可见的 `AI_BRAINSTORM_*` 与 `AI_DAILY_BRAINSTORM_LIMIT`；没有独立模型时按 `AI_TOOL_MODEL`、`AI_MODEL` 回退。Route 使用 Node.js Function、`waitUntil` 和 `maxDuration=300`，仍受 Vercel 套餐实际最大时长约束。
