...
> Phase 5A3-2：长期记忆在现有确定性关键词排序上增加可选的 512 维 Embedding、Supabase pgvector 精确余弦查询与 Hybrid RRF；未配置或运行失败时安全退回关键词召回。这不是外部文件 RAG。详见 `docs/memory-semantic-retrieval.md`。
