...
> Phase 5A1：新增可控的手动长期记忆基础。用户可以在 `/memories` 管理全局或 Persona 专属记忆，也可以从自己的聊天消息保存；聊天仅按确定性规则召回少量启用记忆。详见 `docs/memory-system.md`。
...
Phase 5A1 提供手动长期记忆 CRUD、总开关、聊天消息保存、GLOBAL/PERSONA 作用域、确定性召回和安全 Prompt 注入。该阶段不包含自动提取、向量数据库、Embedding 或 RAG；数据库迁移与真实 RLS 验收步骤见 `docs/memory-system.md`。
